...
define(['jquery', 'lodash', 'backbone', 'vm', 'utils', 'events', 'views/baseField', 'text!templates/subform-layouts/table.html', 'text!templates/subform-layouts/card.html', 'text!templates/notice/confirmation.html', 'bootstrap'], function($, _, Backbone, Vm, Utils, Events, BaseField, tableTemplate, cardTemplate, popoverTemplate) {
    ...
    var AppView = Backbone.View.extend({
        ...
        render: function() {
            ...
            if (hasFileElement || this.options.formSchema.options.addonlywitheditoncreate) {
                // console.log('- _values:', _values);
                // debugger
                if ((_values && _values[0] && _values[0].length === 1) || (this.options.formSchema.options.addonlywitheditoncreate && _options.options.mode === 'update')) {
                    // console.log('adding hideEdit buttons');
            ...
            if (this.options.formSchema.options.addonlywitheditoncreate && _options.options.mode === 'update') {
                // console.log('_options.options:', _options.options);

                var currentFieldFormData = Utils.getModelValueForViewModel(
                    _options.options.formData,
                    _formName
                );
            ...
            $(this.el).html(this.template({
                ...
                addOnlyWithEditOnCreate: this.options.formSchema.options.addonlywitheditoncreate,
                ...
                viewMode: _options.options.mode
            }));
            // Set Up Popover
