...
define([
    ...
    var AppView = BaseFieldView.extend({
        ...
        renderWizardNavBar: function() {
            ...
            _.each(this._steps, function(element, index) {
                ...
                _icon =
                    ...
                    "" :
                    '<i class="icon ' + element.icon + ' icon-3x"></i>';
                _html +=
                    '<li data-target="#wizard_step' +
                    (index + 1) +
                    '" class="' +
                    element["class"] +
                    '">' +
            ...
            if (_width) {
                ...
                _stepWidth = Math.floor(
                    (_width - this._steps.length) / this._steps.length
                );
        ...
        attachedEvents: function() {
            ...
            $(this.el).on(
                this.options.formSchema.name + ".validated",
                this,
                this.validatedForm
            );
            $(this.el).on(
                this.options.formSchema.name + ".postSubmit",
                this,
                this.respondResult
            );
            if (_debugEvents) {
                ...
                console.log("- attached: validated, postSubmit");
            }
        },
            ...
            switch (data.step) {
                ...
                default:
                    e.data.$nextBtn
                        ...
                        .html('Next <i class="icon-arrow-right"></i>');
            }
        },
        /**
        ...
        changeStep: function(e, data) {
            ...
            var targetForm = e.data.el ? jQuery(e.data.el) : null;
            if (DEBUG) {
                console.log("targetForm:", targetForm, "e.data:", e.data);
            ...
            if (targetForm) {
                ...
                Utils.setModelButtonRadioValues(targetForm, e.data);
                Utils.setModelRadioValues(targetForm, e.data);
                Utils.setModelCheckValues(targetForm, e.data);
            }
                    ...
                    switch (data.step) {
                        ...
                        case _numSteps - 1:
                            ...
                            e.data.$nextBtn
                                ...
                                .addClass("btn-info")
                                .html('<i class="icon-envelope-alt"></i> ' + btnSubmit);
                            break;
                    }
        ...
        isStepValid: function(index) {
            ...
            if (typeof this._stepValidated[index] !== "undefined") {
                _.each(this._stepValidated[index], function(element) {
                    $element = $(':input[name="' + element + '"]', that.el).removeClass(
                        "invalid"
                    );
                    if (!$element.is(":checkbox") &&
                        !$element.is(":radio") &&
                        that.model.has(element)
                    ) {
                    ...
                    if (!_elementError) {
                        ...
                        _error = true;
                        if (DEBUG) {
                            console.log("invalid element:", element, 'model:', that.model.toJSON());
                ...
                if (_error) {
                    ...
                    var contentError = Utils.getText(
        ...
        validatedForm: function(e) {
            ...
            if ($form.hasClass("validation_pass")) {
                ...
                _opt.title = _t_1;
                _opt.content =
                    '<i class="icon-spinner icon-spin icon-large"></i> ' + _t_2 + " ...";
                e.data.$nextBtn
                    .attr("disabled", true)
                    .popover(_opt)
                    .popover("show")
        ...
        respondResult: function(e) {
            // console.log("- wizard:respondResult: fired");
            // console.log("- arguments:", arguments);
            // console.log("- e:", e);
            // console.log("- e.data:", e.data);
            if (e && e.data) {
                if (e.data.options) {
                    if (e.data.options.formEvents) {
                        var formEvents = e.data.options.formEvents;
                        // console.log("- formEvents.postSubmit:", formEvents.postSubmit);
                        // console.log(
                        //   "- typeof formEvents.postSubmit:",
                        //   typeof formEvents.postSubmit
                        // );
                        if (
                            formEvents &&
                            formEvents.postSubmit &&
                            typeof formEvents.postSubmit === "function"
                        ) {
                            // formEvents.postSubmit.apply(this, arguments);
                        }
                    }
                }
            }
            window.setTimeout(function() {
                if (
                    ...
                    typeof e.data.$nextBtn !== "undefined"
                ) {
                    e.data.$nextBtn
                        .attr("disabled", false)
                        .popover("destroy")
