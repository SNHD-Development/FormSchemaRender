...
define([
    ...
    Select2Helper,
    countyData,
    externalCreateUserDataTemplate,
    webcamTemplate,
    WebCam,
    Cropper
) {
    ...
    return {
        ...
        changeURLGetTemplateString: function(str, formData) {
            ...
            if (_tok) {
                for (var i in _tok) {
                    ...
                    if (!_tmpArr) {
                        throw (
                            'Unexpected changeURLGetTemplateString, found more than one template variable for "' +
                            _to +
                            '"'
                        );
                    }
                    ...
                    if (
                        ...
                        if (fieldsTokens && fieldsTokens.length > 0) {
                            ...
                            _.each(fieldsTokens, function(key) {
                                ...
                                lastValue =
                                    key in lastValue ? lastValue[key] : lastValue[keyLower];
                                // console.log('- lastValue:', lastValue);
                            });
                            // console.log('- lastValue:', lastValue);
                            if (lastValue) {
                                newStr = newStr.replace("{{" + _tmpArr + "}}", lastValue);
                                continue;
                            }
                        }
                    }
            ...
            $allDatepicker.each(function() {
                ...
                if (
                    ...
                    if (!logicOpts.getvaluefrom) {
                        throw (
                            'In order to use setupDateInput with "DatepickerOptions". Required to have "GetValueFrom" key! (' +
                            _id +
                            ")"
                        );
                    }
                    if (!logicOpts.comparison) {
                        throw (
                            'In order to use setupDateInput with "DatepickerOptions". Required to have "Comparison" key! (' +
                            _id +
                            ")"
                        );
                    }
                        ...
                        switch ($this.attr("data-mindate").toLowerCase()) {
                            ...
                            default:
                                ...
                                if ($this.attr("data-mindate").match(/^\d+$/) != null) {
                                    ...
                                    _options.onRender = function(date) {
                                        return date.valueOf() < nowTemp.valueOf() ?
                                            "disabled" :
                                            "";
                                    };
                                } else {
                        ...
                        var _url = element.url + "?",
                            ...
                            function(e) {
                                ...
                                if (element.options.renderresult && e.data) {
                                    ...
                                    require([
                                        "views/subform-layouts/buttondecision",
                                    ], function(ButtonDecisionView) {
        ...
        isRenderVisibleOn: function(view, value, typeLowerCase) {
            ...
            if (value.options && value.options.visibleon) {
                if (
                    typeLowerCase !== "html" ||
                    value.options.visibleon.forcerenderonread
                ) {
                    ...
                    if (_.isArray(view.options.formData.fields[_visibleOnName])) {
                        ...
                        if (_DEBUG) {
                            ...
                            console.log(
                                "- value.options.visibleon.values:",
                                value.options.visibleon.values
                            );
                            console.log(
                                "- value.options.visibleon.values.indexOf(_lookupValue):",
                                value.options.visibleon.values.indexOf(_lookupValue)
                            );
                        }
                ...
                formModel.fetch({
                    success: function() {
                        ...
                        if (!$currentInput.length) {
                            throw (
                                'Error: Could not be able to find input name "' +
                                inputName +
                                '".'
                            );
                        }
                        ...
                        if (!$span.length) {
                            throw (
                                'Error: Could not be able to find span for "' +
                                inputName +
                                '".'
                            );
                        }
        ...
        setupCreateUserViewModal: function($form, view) {
            ...
            if (!$form ||
                ...
                !view ||
                !view.options.formData ||
                !view.options.formData.externalcreateuserdata
            ) {
                return;
            }
            ...
            var $userModalButton = $actionDiv.find(
                ".btn.external-created-user-modal-btn"
            );
            ...
            var $modal;
            $actionDiv
                .find("#module-external-create-user-data-btn")
                .click(function(e) {
            ...
            $modal = $actionDiv
                .find("#module-external-create-user-data-modal")
                .modal({
                    backdrop: "static",
                    show: false,
                    keyboard: true,
        ...
        setupAjaxCall: function(view, $form) {
            _.each(view._ajaxDataCall, function(element) {
                ...
                _.each(_input, function(elementName) {
                    $form.on("change", ':input[name="' + elementName + '"]', function(
                        e
                    ) {
                        var $thisInput = $(this),
                            _val = $thisInput.val(),
        ...
        setupUrlAjaxCall: function($form, $scope, model) {
            ...
            $urlEndPoint.each(function() {
                ...
                if (_tokens) {
                    ...
                    if (!$this.select2) {
                        throw (
                            'Error: select2 is not yet loaded. Please refresh this page again! (Setup "' +
                            $this.attr("id") +
                            '"")'
                        );
                    }
                                                    ...
                                                    if (value === "") {
                                                        ...
                                                        if (value.length) {
                                                            ...
                                                            $(
                                                                ...
                                                                "subform_" + _listName + ".ajaxUpdate", [value]
                                                            );
                                                        } else if (
                                                            value.data &&
                                                            value.view &&
        ...
        setModelButtonRadioValues: function(el, view, debug) {
            ...
            if (DEBUG) {
                console.log("***** setModelButtonRadioValues *****");
                console.log(el, view);
            }

            var $radiosContainers = el.find(".radio-container");
            if (!$radiosContainers.length) {
                return;
            }
            if (DEBUG) {
                console.log("[*] Debug - setModelRadioValues");
                console.log($radiosContainers);
                console.log(view);
                console.log(view.model);
                console.log(view.model.toJSON());
            }
            $radiosContainers.each(function() {
                var $radios = $(this).find(
                    '.btn-group :input[data-field-type="is-buttons-radio"]'
                );
                if (DEBUG) {
        ...
        setModelRadioValues: function(el, view, debug) {
            ...
            if (DEBUG) {
                console.log("***** setModelRadioValues *****");
                console.log(el, view);
            }
            ...
            if (field.name && field.name === "LogEntries") {
                if (view.options.formData.fields) {
                    var _createItemObj = {
                        ...
                        LogTime: view.options.formData.createddate &&
                            view.options.formData.createddate.$date ?
                            view.options.formData.createddate.$date / 1000 : null,
                        LogUser: view.options.formData.createduser ?
                            view.options.formData.createduser : null,
                    };
        ...
        shouldRenderShowOnUser: function(field) {
            ...
            if (!_.isArray(field.options.showonuser)) {
                throw (
                    "In order to use Options.ShowOnUser for " +
                    field.type +
                    ' with "' +
                    field.name +
                    '" required an array!'
        ...
        addWebcamEvent: function($form, view) {
            ...
            $("body").on(
                "click",
                ".form-render-has-webcam-wrapper .take-snapshot",
                function(e) {
                    if (e && e.preventDefault) {
                        e.preventDefault();
                    }
            ...
            $("body").on(
                "click",
                "#formrender-webcam-modal .formrender-webcam-take-snapshot",
                function(e) {
                    if (e && e.preventDefault) {
                        e.preventDefault();
                    }
            ...
            $("body").on(
                "click",
                "#formrender-webcam-modal-crop .formrender-webcam-retake-snapshot",
                function(e) {
                    if (e && e.preventDefault) {
                        e.preventDefault();
                    }
            ...
            $("body").on(
                "click",
                "#formrender-webcam-modal-crop .formrender-webcam-process-image",
                function(e) {
                    if (e && e.preventDefault) {
                        e.preventDefault();
                    }
                    ...
                    var data = $canvas[0].cropper
                        .getCroppedCanvas()
                        .toDataURL("image/jpeg");

                    var debugPicture = false;

                    $crop.one("hidden", function() {
                        setTimeout(function() {
                            $notificationContent.html(
                                '<div class="alert alert-success">Sending Data, please wait...</div>'
                            );
                            ...
                            if (debugPicture) {
                                var $debugParent = $webcamList.closest(
                                    ".form-render-has-webcam-wrapper"
                                );
                                var $debug = $debugParent.find(
                                    ".form-render-has-webcam-debug"
                                );
                                if (!$debug || !$debug.length) {
                                    $debugParent.append(
                                        '<img class="form-render-has-webcam-debug" src=""/>'
                                    );
                                    $debug = $debugParent.find(".form-render-has-webcam-debug");
                                }
                                $notification.modal("hide");
                                $debug.attr("src", data);
                                console.log(
                                    "- getCanvasData:",
                                    $canvas[0].cropper.getCanvasData()
                                );
                                console.log(
                                    "- getCropBoxData:",
                                    $canvas[0].cropper.getCropBoxData()
                                );
                                return;
                            }
                            sendPhotoData(data);
            ...
            function sendPhotoData(data) {
                ...
                $.ajax({
                    ...
                    success: function() {
                        if (_DEBUG) {
                            console.log("- sendPhotoData: success", arguments);
                        }
                        location.reload(true);
                    },
                    error: function() {
                        if (_DEBUG) {
                            console.log("- sendPhotoData: error", arguments);
                        }
                        $notificationContent.html(
        ...
        setupWebcamWithCrop: function($form, view) {
            ...
            // Set it up!!!
            var html =
                '<div class="form-render-has-webcam-wrapper"><button class="form-render-has-webcam take-snapshot btn btn-primary">Open Webcam</button></div>';
